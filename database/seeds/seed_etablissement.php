...
try {
    if (!tableExists($pdo, 'parametrage_etablissement')) {
        throw new RuntimeException('Table parametrage_etablissement introuvable après reset.');
    }

    $anneeCount = (int) $pdo->query('SELECT COUNT(*) FROM annee_scolaire')->fetchColumn();
    echo "after truncate annee_scolaire count={$anneeCount}\n";
    $anneeRows = $pdo->query('SELECT libelle FROM annee_scolaire')->fetchAll(PDO::FETCH_COLUMN);
    echo "annee_scolaire rows=" . json_encode($anneeRows, JSON_UNESCAPED_UNICODE) . "\n";

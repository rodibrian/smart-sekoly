...
| Remises : application réelle (pourcentage ou montant fixe) avec motif et validation obligatoire par un responsable AVANT application | ✅ Fait (validé) | décision #13 | Implémentée via `RemiseDAO` : statut workflow (attente→approuvee→rejetee), méthodes creer/valider/rejeter/appliquerAFacture, BLOCAGE EFFECTIF si statut≠approuvee (testé), calculs % et montant fixe, audit trail JournalAudit, lien facture-remise via table intermédiaire. Tests passants : RemiseDAOTest.php (CRUD 8/8), RemiseValidationTest.php (TEST NÉGATIF + nominal 2/2), RemiseIntegrationTest.php (chaîne complète 3/3). Migrations schéma appliquées (5 colonnes ajoutées, id_utilisateur_validation nullable). |
